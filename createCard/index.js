...
getRefCropImage(ref) {
    this.refCropImage = ref.init();
  },
getRefHandlingImg(ref) {
    this.refHandlingImg = ref.init();
  },
getRefPhotoFaceDetect(ref) {
    this.refPhotoFaceDetect = ref.init();
  },
getRefCompressImage(ref) {
    this.refCompressImage = ref.init();
  },
getRefSixCard(ref) {
    this.refSixCard = ref.init();
  },
getRefAddtoMyminiprogram(ref) {
    this.refAddtoMyminiprogram = ref.init();
  },Page({
    data: {
        ...
        maxCardWidth: "",
        cardPaddingTop: 25,
        // .card__box上下paddingTop
        cardLeftRightMargin: 10,
        // .card__box距离屏幕的最小左右边距
        cardInfo: {},
        dpr: 1,
        isHandling: false,
        from: "",
        ...
        toolsList: [
            {
                ...
                activeIcon:
                    "/assets/image/id_certificate_icon_background_s@2x.png"
            },
            {
                text: "美颜",
                feature: "beauty",
                ...
                activeIcon:
                    "/assets/image/id_certificate_icon_beautyface_s@2x.png"
            }
        ],
        activeBeautyIndex: 0,
        beautyList: [
        ...
        beautyStatus: 0,
        // 美颜状态 0:关闭 1:处理中 2：开启
        lightStatus: 0,
        // 补光状态 0:关闭 1:处理中 2：开启
        highDefinitionStatus: false,
        // 高清化状态
        ...
        checkRuleList: [],
        // 检测规则列表
        currentCheckRuleText: " ",
        // 当前真正检测项的文字
        checkAltInfo: {},
        // 规则项检测提示  {status,text}
        checkRuleResultShow: false,
        isUseOriginImage: false,
        // 使用原图,不进行抠图
    ...
    async onLoad(query) {
        ...
        this.setData({
            ...
            printbutton_firsttext:
                getApp().globalData.setting.printbutton_firsttext ||
                "冲印纸质版",
            printbutton_color:
        ...
        _my.onUserCaptureScreen(function() {
            mwx.report("createcard_screenshot");
        });

        this.setData({
            redPointShow: _my.getStorageSync("isRedPointShow") ? false : true
        });
    ...
    onUnload() {
        ...
        this.paymentTimer && clearInterval(this.paymentTimer); // wx.offAppHide();
    },
    ...
    showHandingImageAnimate() {
        ...
        let loadingComponent = this.refHandlingImg();
    ...
    async retouchOriginImage() {
        try {
            ...
            this.originFileNameList.push(fileName); // 5. 进行抠图
    ...
    async faceDetect() {
        ...
        try {
            ...
            if (!anchors.length && !useLocalFace) {
                mwx.action.setStatisticImageUpload({
                    poi: 2001
                });
                ...
                try {
                    ...
                    try {
                        if (
                            getApp().globalData.isSupportLocalFaceDetect !==
                            false
                        ) {
                            mwx.log.info("使用本地人脸检测"); // 本地处理

                            this.setData({
                                faceDetectComponentShow: true
                            });
                            await mwx.pollCheck(() =>
                                this.refPhotoFaceDetect()
                            );
                            let c = this.refPhotoFaceDetect();
                        ...
                        mwx.log.error(error); // throw new Error('人脸检测失败，请重试');
                    }
                }
            }
            ...
            let cropInfo;
            let afterHandlerImage; // 2. 人脸区域裁剪
            ...
            let faceAngleCrop_step2 = async ({ times = 1 } = {}) => {
                ...
                afterHandlerImage = await this.refCropImage().crop({
            ...
            let scoreAndEnhance_step3 = async ({ times = 1 } = {}) => {
                ...
                let faceEnhance = async () => {
                    ...
                    this.showHandingImageAnimate(); // 更新修改模糊项检测结果
            ...
            let retouchAndAdjustPosition_step4 = async ({ times = 1 } = {}) => {
                ...
                if (
                    ...
                    let uploadRes = await mwx.api.checkImage(
                        afterHandlerImage,
                        this,
                        {
                            origin: 1,
                    ...
                    if (uploadRes.checkStatus != 1) {
                        ...
                        this.goFailBack({
                            type: uploadRes.checkStatus == -1 ? 3 : 4
                        });
                        return;
                    }
                }
                ...
                if (checkStatus == 1) {
                    ...
                    let rightSpaceWidth = info.width - x2; // debugger;
                    ...
                    if (
                        ...
                        res.url = await this.refCropImage().adjustPosition({
                            imageSrc: res.url,
                            width: info.width,
                            height: info.height,
                            offsetTop,
    ...
    resolveCheckTask({ action, status = "pass", msg, desc = "" }) {
        ...
        checkRuleList.forEach(v => {
            if (v.action == action) {
                this.setData({
                    currentCheckRuleText: `${loadingTextMap[action] ||
                        "合格性检测中"}，请稍等`
                });
                v.status = status;
                v.msg = msg;
    ...
    async checkIfCanCreateSixCard() {
        try {
            ...
            let { width, height } = await mwx.getImageInfo(
    ...
    async useSixCardComponentCreate(imgSrc, opt) {
        ...
        await mwx.pollCheck(() => this.refSixCard());
        let config = {
            ...opt,
            h: cardInfo.h,
            w: cardInfo.w,
        ...
        return this.refSixCard()
            ...
            .finally(() => {
                ...
                _my.setStorageSync("sixCardCreateInfo", { ...config, imgSrc });
            });
    },
    ...
    async createSixCardImage(imgSrc, isOriginImage) {
        ...
        if (data) {
            ...
            proTask = this.useSixCardComponentCreate(imgSrc, {
                ...
                cardBottomText,
                fontSize: Math.round((cardInfo.w / cardWidth) * 16),
                // 295:一寸的宽  12：一寸的字体大小
                textBoxHeight: Math.round((cardInfo.w / cardWidth) * 24)
            }).then(sixCardImageSrc => {
                cache.set(key, sixCardImageSrc); // let cardInfo = mwx.action.getCardInfo() || {};
    ...
    async initView() {
        ...
        let navbarHeight = app.globalData.navbarHeight; // .footer的高度

        let footerHeight =
            (app.globalData.isIPX ? 388 : 320) / app.globalData.px2rpx;
        let containerHeight = windowHeight - navbarHeight - footerHeight; // 顶部.switch__btns高度的height为88,marginTop=24
        ...
        console.log(
            `maxCardHeight=${maxCardHeight},maxCardWidth=${maxCardWidth}`
        );
        this.setData({
            maxCardHeight,
            maxCardWidth
        ...
        let cardTransformRatio = cardInfo.unit == "px" ? 1 : dpiRatio; // 实际绘制的证件宽高

        let actualCardWidth = cardInfo.width * cardTransformRatio;
        let actualCardHeight = cardInfo.height * cardTransformRatio;
        let containerRatio = maxCardWidth / maxCardHeight; // 界面中的证件宽高
    ...
    async createSingleCard({ type = "origin" } = {}) {
        ...
        if (data) {
            ...
            if (type == "origin") {
                _my.setStorageSync("highOriginConfig", config);
            ...
            _my.setStorageSync("singleCardCreateInfo", {
                ...config,
    ...
    async minifyImage(originImageSrc, sizeRange) {
        ...
        let quality = 50;
        let startQuality = 0;
        let endQuality = 100; // 图片质量无法增大，所以图片小于最大尺寸的都直接返回
        ...
        if (size <= max) {
            ...
            let minInfo = await handle(0); // 最小压缩质量都大于最大要求的尺寸,那就直接返回
    ...
    async goBack() {
        ...
        let task = this.createSingleCard({
            type: "origin"
        });
        mwx.showModal({
            title: "提示",
    ...
    uploadUnSaveCard() {
        let originFileNameList = _my.getStorageSync("originFileNameList") || [];
    ...
    async switchView(e) {
        ...
        if (ind == 1) {
            ...
            _my.setStorageSync("isRedPointShow", 1);
    ...
    launchDanmu() {
        ...
        let launch = time => {
            let waitTime = time || Math.floor(Math.random() * 3 + 7); // console.log('waitTime', waitTime);
    ...
    savePaperCard: mwx.util.ignoreMultiClick(async function() {
        ...
        _my.reportEvent("createcard_clickprint", {
        ...
        _my.setStorageSync("buyCardInfo", buyCardInfo);
    ...
    saveCard: mwx.util.ignoreMultiClick(async function() {
        ...
        mwx.report("createcard_clicksave"); // 换装的处理逻辑
        ...
        if (this.data.activeClothId) {
            ...
            if (this.data.isIos) {
                ...
                mwx.navigateTo({
                    url: `/packageA/submitChangeClothOrder/index?type=${
                        this.query.from
                    }&sixCardCount=${this.sixCardCount ||
                        ""}&useOriginImageSize=${
                        this.useOriginImageSize ? 1 : ""
        ...
        if (isVip) {
            ...
            if (this.data.activeViewTypeIndex == 0) {
                ...
                let cardImage = mwx.action.getCardImage(); // 先上传图片，不生成记录

                let r = await mwx.service.uploadFinishCreateSingleCard({
                    cardImage
                });
                mwx.navigateTo({
                    url: `/packageA/saveCompose/index?needCreateHistory=1&fileName=${r.fileName}`
                });
            }
        }
    }),
    ...
    async createCard({ cardProductId, loadingShow = true }) {
        ...
        mwx.action.setCardInfo({
            ...
            bgColor,
            colorImgUrl: itemColor.colorImgUrl || "",
            // 渐变色才有图片
            faceBeauty: this.data.beautyStatus == 2 ? 1 : 0,
            cardProductId,
            faceEnhance: this.useFaceEnhance ? 1 : 0,
            alphaFileName:
                this.currentAlphaFileName || this.alphaFileName || "",
        ...
        try {
            let taskList = [];
            let task = Promise.resolve(); // debugger;
    ...
    clickSaveCardBox: mwx.util.ignoreMultiClick(async function(e) {
        ...
        switch (e.detail) {
            ...
            case "buy":
                ...
                _my.navigateTo({
                    url: "/packageA/submitOrder/index"
                });
            ...
            case "appreciation":
                ...
                try {
                    let res = await mwx.service.createCardHistory({
                        status: 0,
                        cardProductId: this.data.boxData.appreciationProduct
                            .product,
                        payWay: 9,
                        scene: 39
                    });
            ...
            case "collect":
                ...
                if (this.isAddToMyMiniProgram) {
                    ...
                    await mwx.pollCheck(() =>
                        this.refAddtoMyminiprogram()
                    );
                    this.refAddtoMyminiprogram().show();
                    clearInterval(this.addToMyMiniProgramTimer);
                    this.addToMyMiniProgramTimer = setInterval(() => {
                        _my.checkIsAddedToMyMiniProgram({
                            success: res => {
                                if (res.added) {
                                    clearInterval(this.addToMyMiniProgramTimer);
    ...
    async previewSixCard() {
        ...
        if (!this.sixCardImageSrcWithWatermark) {
            //
            let r = await this.useSixCardComponentCreate(
                this.data.cardImageSrc,
                {
                    ...this.getActiveColorInfo(),
                    type: "preview",
                    ...
                    fontSize: Math.round((cardInfo.w / cardWidth) * 12),
                    // 295:一寸的宽  12：一寸的字体大小
                    textBoxHeight: Math.round((cardInfo.w / cardWidth) * 24)
                }
            );
  ...
  */
                        ...
                        beautyStatus:
                            type == "beauty"
                                ? ind == 0
                                    ? 0
                                    : 2
                                : this.data.beautyStatus,
                        // type有值就设置，没有用默认的美颜状态
                        lightStatus:
                            type != "beauty"
                                ? ind == 0
                                    ? 0
                                    : 2
                                : this.data.lightStatus
                    }
                });
            } catch (error) {
                mwx.log.error(error);
    ...
    toBase64(imagePath) {
        return new Promise((resolve, reject) => {
            _my.getFileSystemManager().readFile({
                filePath: imagePath,
                // 图片文件路径
                encoding: "base64",
                success: res => {
    ...
    async getAutoBeauty(fileName) {
        ...
        try {
            ...
            _my.setStorageSync("originFileNameList", this.originFileNameList);

            let url = (await mwx.downloadFile(r.url)).tempFilePath; // 没有补光时，需要调整位置
    ...
    async getAddLight(fileName) {
        ...
        try {
            ...
            let url = (await mwx.downloadFile(r.url)).tempFilePath; // 没有美颜时，需要调整位置
    ...
    async getAdjustPosition(r) {
        ...
        //   r = await this.refCropImage().adjustPosition({
        //     imageSrc: r,
        //     ...this.adjustPositionParams
        //   });
        // }
    ...
    async switchToOrigin() {
        ...
        if (
            ...
            let r = await this.createSixCardImage(
                this.data.originCardImageSrc,
                true
            );
            this.setData({
    ...
    goFailBack({ type }) {
        ...
        _my.reportEvent("createcard_imageinvalid", {
            cardfail_reason: type
        });

        try {
            mwx.log.warn(
                "证件照制作失败：stepRecordList=" +
                    JSON.stringify(this.stepRecordList)
            );
    ...
    async confirmCardTextBox(e) {
        ...
        mwx.report("createcard_addtext"); // 清除已生成的证件照
    ...
    closeAddToMyMiniProgram: mwx.util.ignoreMultiClick(function() {
        ...
        return this.checkAddToMyMiniProgram().then(r => {
            ...
            this.goFinishCard();
            this.refAddtoMyminiprogram().close();
        });
    }),
    ...
    goFinishCard(str = "") {
        ...
        if (showClothAlt) {
            _my.setStorageSync("clothAltInfo", {
        ...
        mwx.navigateTo({
            url: `/packageA/finishCard/index?type=${
                this.query.from
            }&sixCardCount=${this.sixCardCount || ""}&useOriginImageSize=${
                this.useOriginImageSize ? 1 : ""
            }&activeClothId=${this.data.activeClothId ||
    ...
    useCustomColor() {
        ...
        mwx.navigateTo({
            url: "/packageA/customColor/index"
        });
        mwx.event.once("useCustomColor", res => {
            let colorList = [...this.data.colorList, res]; // 从原图切换到其他底色
    ...
    async finishEvaluate({ detail }) {
        ...
        mwx.api.wechatRating({
            appletStar: detail.score
        });
        mwx.action.setUserInfo({
            newUser: 0
        });

        if (detail.score >= 3) {
            try {
                await mwx.setClipboardData({
                    data: detail.evaluateText
                });
                mwx.Toast(" ");
    ...
    async switchCloth(e) {
        ...
        let cache_cardImageSrc; // 清除已生成的证件照

        this.clearExistCard(); // debugger;
        ...
        if (!e.detail.noCloth) {
            ...
            _my.setStorageSync("originFileNameList", this.originFileNameList); // 更新未特效处理的换装原图
    ...
    async showClothTool() {
        ...
        this.prevClothId = this.data.activeClothId; // 第一次打开时，展示推荐的换装
    ...
    createSearchEffectStatus() {
        let cache = {}; // 存储 origin、beauty、light的fileName和url
        ...
        return {
            setStatus({ type, activeColorIndex, data }) {
                ...
                let cacheKey = `isUseOriginImage=${activeColorIndex ==
                    -1},type=${type}`;
                let fakeCache = { ...cache };
                console.log("\n\n========\n更新前", fakeCache);
                cache[cacheKey] = { ...(cache[cacheKey] || {}), ...data };
    ...
    async clothFinishPayment(e) {
        ...
        try {
            ...
            let cardImageSrc = (await mwx.downloadFile(res.alphaUrl))
                .tempFilePath;
            this.setData({
                cardImageSrc
            });
            this.clearExistCard();
            await this.createCard({
                cardProductId: e.detail.cardProductId
            });
            console.log("res", res);
            this.setData({
                clothSaveBoxShow: false
    ...
    checkIfNeedShowClothAlt() {
        let isClothAltShow = _my.getStorageSync("isClothAltShow");

        let showChangeClothPercent = getApp().globalData.setting
            .showChangeClothPercent; // 抽样比例
    ...
    async showAutoChangeClothBox() {
        ...
        startTime = new Date(); // let clothId = 'api_20240204-09'; // 后台返回的匹配衣服
        ...
        this.confirmClothCallback = () => {
            ...
            _my.setStorageSync("isClothAltShow", 1);
        };
    ...
    async switchClothBeauty(e) {
        ...
        try {
            ...
            this.setData({
                originCardImageSrc: cardImageSrc,
                cardImageSrc // beautyStatus: beauty ? 2 : 0,
